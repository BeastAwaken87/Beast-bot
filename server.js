...
client.on
client.on("ready", () => {
    client.user.setActivity("BEASTU SIR OP", { type: "LISTENING"})
})
...
console.log("Subscribe on Rogmitop https://www.youtube.com/channel/UCEXfHVE2hLNP8CdgdlUTt2Q")
...
client.on("message", async message => {
  if (message.channel.name == "abotchat") {
    ...
    message.channel.startTyping();
    if (!message.content) return message.channel.send("Please say something.");
    scb.chat({message: message.content, name: client.user.username, owner:"Rogmitop", user: message.author.id, language:"auto"}).then(reply => {
